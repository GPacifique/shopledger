    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                <button onclick="window.open('{{ route('sales.print', $sale) }}?print=1', '_blank')" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-200 transition">
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-xl mb-6 print:shadow-none">
                ...
                <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-8 text-white">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-green-100 text-sm uppercase tracking-wider">{{ __('Sale Receipt') }}</p>
                            <p class="text-3xl font-bold mt-1">#{{ str_pad($sale->id, 5, '0', STR_PAD_LEFT) }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-green-100 text-sm">{{ __('Total Amount') }}</p>
                ...
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wider">{{ __('Date') }}</p>
                            <p class="font-medium text-gray-900">{{ $sale->sale_date->format('M d, Y') }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wider">{{ __('Time') }}</p>
                            <p class="font-medium text-gray-900">{{ $sale->created_at->format('h:i A') }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wider">{{ __('Payment') }}</p>
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-b border-gray-200">
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider bg-gray-50">{{ __('Product') }}</th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider bg-gray-50">{{ __('Qty') }}</th>
                                <th class="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider bg-gray-50">{{ __('Price') }}</th>
                                <th class="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider bg-gray-50">{{ __('Total') }}</th>
                            </tr>
                        </thead>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                    <div class="flex justify-end">
                        <div class="w-64 space-y-2">
                            ...
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">{{ __('Tax') }} (0%)</span>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-3 gap-6">
                        <div class="bg-green-50 rounded-xl p-4 text-center">
                            <p class="text-sm text-green-600 font-medium">{{ __('Revenue') }}</p>
                            <p class="text-2xl font-bold text-green-700 mt-1">{{ rwf($sale->total_amount) }}</p>
                        </div>
                        <div class="bg-red-50 rounded-xl p-4 text-center">
                            <p class="text-sm text-red-600 font-medium">{{ __('Cost of Goods') }}</p>
                        ...
                        <div class="bg-{{ $profit >= 0 ? 'emerald' : 'rose' }}-50 rounded-xl p-4 text-center">
                            <p class="text-sm text-{{ $profit >= 0 ? 'emerald' : 'rose' }}-600 font-medium">{{ __('Net Profit') }}</p>
                            ...
                            @if($sale->total_amount > 0)
                                <p class="text-xs text-{{ $profit >= 0 ? 'emerald' : 'rose' }}-500 mt-1">
                                    {{ number_format(($profit / $sale->total_amount) * 100, 1) }}% {{ __('margin') }}
                                </p>
                            @endif
                        </div>
                    </div>
                    ...
                    <!-- Per Item Breakdown -->
                    <div class="mt-6">
                        <p class="text-sm font-medium text-gray-700 mb-3">{{ __('Profit by Item') }}</p>
                        <div class="space-y-2">
                            @foreach($sale->items as $item)
                            @php
