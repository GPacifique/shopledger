            ...
            <nav class="relative z-10 px-6 py-4 lg:px-12">
                <div class="flex items-center justify-between max-w-7xl mx-auto">
                    ...
                    <div class="flex items-center space-x-4">
                        <!-- Language Switcher for Welcome Page -->
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.outside="open = false" class="inline-flex items-center px-3 py-2 bg-white/20 backdrop-blur-sm text-white rounded-xl font-semibold hover:bg-white/30 transition">
                                @php
                                    $flags = ['en' => '🇬🇧', 'fr' => '🇫🇷', 'rw' => '🇷🇼', 'sw' => '🇹🇿'];
                                @endphp
                                <span class="mr-1">{{ $flags[app()->getLocale()] ?? '🇬🇧' }}</span>
                            ...
                            <div x-show="open" x-transition class="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-lg overflow-hidden" style="display: none;">
                                @foreach(['en' => ['English', '🇬🇧'], 'fr' => ['Français', '🇫🇷'], 'rw' => ['Kinyarwanda', '🇷🇼'], 'sw' => ['Kiswahili', '🇹🇿']] as $code => $lang)
                                    <a href="{{ route('language.switch', $code) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 {{ app()->getLocale() === $code ? 'bg-indigo-100' : '' }}">
                                        <span class="mr-2">{{ $lang[1] }}</span>{{ $lang[0] }}
                                    </a>
                                @endforeach
                            </div>
                        </div>
