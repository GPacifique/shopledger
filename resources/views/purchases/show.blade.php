        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-lg text-gray-900">
                                ...
                                @else
                                    <span class="text-gray-400">{{ __('No supplier') }}</span>
                                @endif
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Created By') }}</dt>
                            <dd class="mt-1 text-lg text-gray-900">{{ $purchase->creator->name ?? __('Unknown') }}</dd>
                        </div>
                        <div>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg mb-6">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">{{ __('Purchase Items') }}</h3>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Line Total') }}</th>
                            </tr>
                        </thead>
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-right text-sm font-medium text-gray-900">{{ __('Total') }}:</td>
            ...
            <div class="flex justify-between items-center">
                <div class="text-sm text-gray-500">
                    {{ __('Created on') }} {{ $purchase->created_at->format('M d, Y \a\t h:i A') }}
                </div>
                <form action="{{ route('purchases.destroy', $purchase) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to delete this purchase? This will reverse all stock changes.') }}')">
                    @csrf
                    @method('DELETE')
