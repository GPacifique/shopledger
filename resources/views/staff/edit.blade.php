            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-xl">
                <form method="POST" action="{{ route('staff.update', $staff) }}" class="p-6 space-y-6">
                    ...
                    <div class="text-center pb-6 border-b border-gray-200">
                        ...
                        <p class="mt-2 text-sm text-gray-500">Editing {{ $staff->email }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="role" id="role" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="seller" {{ old('role', $staff->role) == 'seller' ? 'selected' : '' }}>Seller</option>
                        ...
                        <div>
                            ...
                            <select name="account_status" id="account_status" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="active" {{ old('account_status', $staff->account_status) == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="suspended" {{ old('account_status', $staff->account_status) == 'suspended' ? 'selected' : '' }}>Suspended</option>
                            </select>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <p class="text-sm text-yellow-700">
                                    <strong>Change Password (Optional)</strong> - Leave blank to keep current password
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="password" name="password" id="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Leave blank to keep current">
                            @error('password')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" id="password_confirmation" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Repeat new password">
                        </div>
                    </div>
                ...
                <!-- Hidden delete form -->
                <form id="delete-form" action="{{ route('staff.destroy', $staff) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this staff member? This action cannot be undone.')">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
